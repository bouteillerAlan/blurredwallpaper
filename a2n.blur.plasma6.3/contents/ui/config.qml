...
ColumnLayout {
  ...
  property bool dynamicModeSupported: typeof PlasmaWallpaper.DynamicMode !== "undefined"
  ...
  function dynamicModeValue(name: string): int {
    // The enum values are only referenced inside the function body so that they
    // are never evaluated on Plasma versions where DynamicMode does not exist
    return root.dynamicModeSupported ? PlasmaWallpaper.DynamicMode[name] : 0;
  }
  ...
  // Only set dynamicMode when the running Plasma version supports it (>= 6.1)
  Binding {
    target: imageWallpaper
    property: "dynamicMode"
    value: root.cfg_DynamicMode
    when: root.dynamicModeSupported
  }
  ...
  Kirigami.FormLayout {
    ...
    RowLayout {
      visible: !cfg_IsSlideshow && root.dynamicModeSupported
      ...
      QtControls2.ComboBox {
        ...
        model: [
          {
            dynamicMode: root.dynamicModeValue("Automatic"),
          ...
          {
            dynamicMode: root.dynamicModeValue("DayNight"), text: i18ndc("plasma_wallpaper_org.kde.image", "@item:inlistbox part of a sentence: 'Switch dynamic wallpapers'", "Based on the day-night cycle")
          },
          {
            dynamicMode: root.dynamicModeValue("AlwaysLight"),
            text: i18ndc("plasma_wallpaper_org.kde.image", "@item:inlistbox", "Always use light variant")
          },
          {
            dynamicMode: root.dynamicModeValue("AlwaysDark"),
